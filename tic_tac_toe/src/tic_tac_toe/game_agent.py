...
def getPlayerMove(board):
    ...
    while move not in '0 1 2 3 4 5 6 7 8'.split() or not isSpaceFree(board, int(move)):
        ...
        move = raw_input()
    return int(move)
    #TODO: getPlayerMove is listening to changes in the board state
...
while True:
    # Reset the board
    theBoard = [' '] * 10
    emptyLetter, playerLetter, computerLetter = [' ', 'X', 'O'] #inputPlayerLetter() #Hardcode - player is always X/Red/1
    turn = 'player' #whoGoesFirst() #Hardcode - player always goes first
